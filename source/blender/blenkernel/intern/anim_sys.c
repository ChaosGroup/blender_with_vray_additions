...
void BKE_animsys_evaluate_animdata_pyntree(struct Depsgraph *depsgraph, struct Main *main, struct Scene *scene, float ctime, short recalc)
{
	for (bNodeTree *ntree = main->nodetree.first; ntree; ntree = ntree->id.next) {
		if (ntree->type == NTREE_CUSTOM) {
			BKE_animsys_evaluate_animdata(depsgraph, scene, &ntree->id, ntree->adt, ctime, recalc);
		}
	}
}
