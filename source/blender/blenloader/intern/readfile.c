	...
	for (loop = prop->data.group.first; loop; loop = loop->next) {
		...
		case IDP_ID: /* DatablockProperty */
			...
			if (strcmp(loop->name, "ntree") == 0) {
				...
				FOREACH_NODETREE_BEGIN(main, nodetree, ntreeIter) {
...
static void lib_link_action(FileData *fd, Main *main)
{
	for (bAction *act = main->action.first; act; act = act->id.next) {
		if (act->id.tag & LIB_TAG_NEED_LINK) {
			// NOTE(VRAY): do we need to do this
...
static void lib_link_workspaces(FileData *fd, Main *bmain)
{
	for (WorkSpace *workspace = bmain->workspaces.first; workspace; workspace = workspace->id.next) {
		...
		IDP_LibLinkProperty(id->properties, fd, bmain);
		id_us_ensure_real(id);
...
static void lib_link_view_layer(FileData *fd, Library *lib, ViewLayer *view_layer, Main *main)
{
	...
	IDP_LibLinkProperty(view_layer->id_properties, fd, main);
}
...
static void lib_link_collection(FileData *fd, Main *main)
{
	for (Collection *collection = main->collection.first; collection; collection = collection->id.next) {
		if (collection->id.tag & LIB_TAG_NEED_LINK) {
			...
			IDP_LibLinkProperty(collection->id.properties, fd, main);
			...
			if (collection->view_layer) {
				lib_link_view_layer(fd, collection->id.lib, collection->view_layer, main);
			}
#endif
	...
	for (Scene *sce = main->scene.first; sce; sce = sce->id.next) {
		if (sce->id.tag & LIB_TAG_NEED_LINK) {
			...
			for (ViewLayer *view_layer = sce->view_layers.first; view_layer; view_layer = view_layer->next) {
				lib_link_view_layer(fd, sce->id.lib, view_layer, main);
			}
...
static void lib_link_lightprobe(FileData *fd, Main *main)
{
	for (LightProbe *prb = main->lightprobe.first; prb; prb = prb->id.next) {
		if (prb->id.tag & LIB_TAG_NEED_LINK) {
			IDP_LibLinkProperty(prb->id.properties, fd, main);
...
static void lib_link_all(FileData *fd, Main *main)
{
	...
	// NOTE(VRAY): consider removing this and keeping changes from master
	i = set_listbasepointers(main, lbarray);
	while (i--) {
...
struct Main *BLO_load_main_from_file(const char *filepath)
{
	...
	bfd = BLO_read_from_file(filepath, BLO_READ_SKIP_NONE, NULL);
	if (bfd) {
		bmain = bfd->main;
